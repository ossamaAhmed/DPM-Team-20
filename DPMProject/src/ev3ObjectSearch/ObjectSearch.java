	...
	public void move() {
		...
		if (usPoller.getDistance() < 0.1) {
			while (usPoller.getDistance() < 0.25) {
				nav.setSpeeds(-1*MEDIUM,-1*MEDIUM);
			}
		}

		if (isBlackListed()) {
			nav.setSpeeds(MEDIUM, HIGH);
		}
			...
			nav.setSpeeds(HIGH, MEDIUM);
		} else {
			nav.setSpeeds(HIGH, HIGH);
